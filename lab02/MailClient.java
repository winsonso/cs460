...
*    Winson So
* Summary:
*    This program is to implement a mail user agent (MUA) that sends mail 
*    to remote host
************************************************************************** */
...
public class MailClient extends Frame
{
   ...
   class SendListener implements ActionListener
   {
      ...
      {
         System.out.println("GUI preparing to send mail ...");
...
class SMTPConnection
{
   ...
   public SMTPConnection(Envelope envelope) throws IOException
   {
      ...
      if (parseReply(reply) != 220) {
         throw new IOException("Expected: " + 220 + " from server, got: " + parseReply(reply));
      }
      
      System.out.println("Client received reply: "+ reply);
      String hostAddr = (InetAddress.getLocalHost()).getHostAddress();
      System.out.println("Client sending handshake command: HELO " + hostAddr);
   ...
   public void send(Envelope envelope) throws IOException
   {
      ...
      System.out.println("Client sending mail from: MAIL FROM: <" + envelope.Sender + ">");
      sendCommand("MAIL FROM: <" + envelope.Sender + ">", 250);
      System.out.println("Client sending rcpt to: RCPT TO: <" + envelope.Recipient + ">");
      sendCommand("RCPT TO: <" + envelope.Recipient + ">", 250);
      System.out.println("Client sending DATA flag: DATA");
      sendCommand("DATA",354);
      System.out.println("Client sending message body: From: " + envelope.Sender);
   ...
   private void sendCommand(String command, int rc) throws IOException
   {
      ...
      if (parseReply(reply) != rc) {
         throw new IOException("Expected: " + rc + " from server, got: " + parseReply(reply));
      }
